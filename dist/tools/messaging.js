export function messagingTools(client) {
    return [
        {
            name: "whatsapp_send_message",
            description: "Send a text message to a WhatsApp contact or group. Use phone number (with country code) or JID format.",
            inputSchema: {
                type: "object",
                properties: {
                    to: {
                        type: "string",
                        description: "Phone number with country code (e.g., '1234567890') or WhatsApp JID (e.g., '1234567890@s.whatsapp.net') or group ID",
                    },
                    message: {
                        type: "string",
        ...
        },
        {
            ...
            handler: async (args) => {
                const result = await client.sendImageMessage(args.to, args.image_url, args.caption);
                return {
                    success: true,
                    messageId: result?.key?.id,
                    to: args.to,
        ...
        },
        {
            ...
            handler: async (args) => {
                const result = await client.sendVideoMessage(args.to, args.video_url, args.caption);
                return {
                    success: true,
                    messageId: result?.key?.id,
                    to: args.to,
        ...
        },
        {
            ...
            description: "Send an audio file or voice note to a WhatsApp contact or group",
            inputSchema: {
                type: "object",
                properties: {
                    to: {
            ...
            handler: async (args) => {
                const result = await client.sendAudioMessage(args.to, args.audio_url, args.is_voice_note || false);
                return {
                    success: true,
                    messageId: result?.key?.id,
                    to: args.to,
        ...
        },
        {
            ...
            handler: async (args) => {
                const result = await client.sendDocumentMessage(args.to, args.document_url, args.filename, args.mimetype);
                return {
                    success: true,
                    messageId: result?.key?.id,
                    to: args.to,
        ...
        },
        {
            ...
            handler: async (args) => {
                const result = await client.sendLocationMessage(args.to, args.latitude, args.longitude, args.name, args.address);
                return {
                    success: true,
                    messageId: result?.key?.id,
                    to: args.to,
        ...
        },
        {
            ...
            inputSchema: {
                ...
                properties: {
                    ...
                    contact_number: {
                        type: "string",
                        description: "Phone number of the contact to send (without + or spaces)",
                    },
                },
                required: ["to", "contact_name", "contact_number"],
            },
            handler: async (args) => {
                const result = await client.sendContactMessage(args.to, args.contact_name, args.contact_number);
                return {
                    success: true,
                    messageId: result?.key?.id,
                    to: args.to,
        ...
        },
        {
            ...
            handler: async (args) => {
                const result = await client.sendQuotedMessage(args.to, args.reply_text, args.message_id);
                return {
                    success: true,
                    messageId: result?.key?.id,
        ...
        },
        {
            ...
            handler: async (args) => {
                await client.deleteMessage(args.to, args.message_id, args.for_everyone !== false);
                return {
                    success: true,
        ...
        },
        {
            ...
            handler: async (args) => {
                const result = await client.sendButtonMessage(args.to, args.text, args.footer || "", args.buttons);
                return {
                    success: true,
                    messageId: result?.key?.id,
                };
        ...
        },
        {
            ...
            handler: async (args) => {
                const result = await client.sendListMessage(args.to, args.text, args.footer || "", args.title || "", args.button_text, args.sections);
                return {
                    success: true,
                    messageId: result?.key?.id,
                };
        ...
        },
        {
            ...
            handler: async (args) => {
                const messages = await client.getChatMessages(args.jid, Math.min(args.limit || 20, 100));
                return {
                    chat: args.jid,
