...
export function groupTools(client: WhatsAppClient): ToolDefinition[] {
  return [
    {
      ...
      handler: async (args: { name: string; participants: string[] }) => {
...

    {
      ...
      handler: async (args: { group_id: string; participants: string[] }) => {
        const result = await client.addGroupParticipants(
          args.group_id,
          args.participants
        );
        return {
...

    {
      ...
      handler: async (args: { group_id: string; participants: string[] }) => {
        const result = await client.promoteGroupParticipants(
          args.group_id,
          args.participants
        );
        return {
...

    {
      ...
      handler: async (args: { group_id: string; participants: string[] }) => {
        const result = await client.demoteGroupParticipants(
          args.group_id,
          args.participants
        );
        return {
...

    {
      ...
      handler: async (args: { group_id: string; name: string }) => {
...

    {
      ...
      handler: async (args: { group_id: string; description: string }) => {
      ...
      inputSchema: {
        ...
        properties: {
          invite_code: {
            type: "string",
            description:
              "Invite code or full invite link (https://chat.whatsapp.com/CODE)",
          },
        },
        required: ["invite_code"],
      },
      handler: async (args: { invite_code: string }) => {
        ...
        if (code.includes("chat.whatsapp.com/")) {
          try {
            const url = new URL(
              code.startsWith("http") ? code : `https://${code}`
            );
