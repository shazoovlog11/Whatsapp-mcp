...
export function statusTools(client: WhatsAppClient): ToolDefinition[] {
  return [
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          background_color: {
            type: "string",
            description:
              "Background color in hex format (e.g., '#075E54')",
            default: "#075E54",
          },
        },
        required: ["text"],
      },
      handler: async (args: { text: string; background_color?: string }) => {
        await client.sendTextStatus(
          args.text,
          args.background_color || "#075E54"
        );
        return {
          success: true,
          text: args.text,
...

    {
      ...
      handler: async (args: { image_url: string; caption?: string }) => {
