...
export function utilityTools(client: WhatsAppClient): ToolDefinition[] {
  return [
    {
      ...
      handler: async () => {
        ...
        if (!qrCode) {
          return {
            status,
            message:
              "QR code not available yet. WhatsApp may still be connecting.",
            qrCode: null,
            qrCodeDataUrl: null,
          };
        }
...

    {
      ...
      handler: async (args: { jid: string; archive?: boolean }) => {
...

    {
      ...
      handler: async (args: { jid: string; duration_hours?: number }) => {
...

    {
      ...
      handler: async (args: { jid: string; pin?: boolean }) => {
...

    {
      ...
      description:
        "Convert a phone number to WhatsApp JID format for use in other tools",
      inputSchema: {
        type: "object",
        properties: {
          phone: {
      ...
      handler: async (args: { phone: string; is_group?: boolean }) => {
